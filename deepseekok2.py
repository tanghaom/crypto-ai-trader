...
class ModelContext:
    ...
    def _create_web_state(self) -> Dict:
        ...
        return {
            ...
            "ai_model_info": {"provider": self.provider, "model": self.model_name, "status": "unknown", "last_check": None, "error_message": None},
            "account_summary": {"total_balance": 0, "available_balance": 0, "total_equity": 0, "total_unrealized_pnl": 0},
            "account_info": {},
            "balance_history": [],
        }
...
def get_model_metadata() -> List[Dict[str, str]]:
    return [
        {"key": key, "display": ctx.display, "model_name": ctx.model_name, "provider": ctx.provider, "sub_account": getattr(ctx, "sub_account", None)}
        for key, ctx in MODEL_CONTEXTS.items()
    ]
...
RANGE_PRESETS = {"1d": timedelta(days=1), "7d": timedelta(days=7), "15d": timedelta(days=15), "1m": timedelta(days=30), "1y": timedelta(days=365)}
...
def get_overview_payload(range_key: str = "1d") -> Dict:
    ...
    return {
        ...
        "aggregate": {"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "total_equity": total_equity, "ratios": model_ratios},
    }
...
def get_market_trend(df):
    ...
    try:
        ...
        return {"short_term": trend_short, "medium_term": trend_medium, "macd": macd_trend, "overall": overall_trend, "rsi_level": df["rsi"].iloc[-1]}
    except Exception as e:
        print(f"趋势分析失败: {e}")
        return {}
...
def get_symbol_ohlcv_enhanced(symbol, config):
    ...
    try:
        # 获取K线数据
        ohlcv = exchange.fetch_ohlcv(symbol, config["timeframe"], limit=config["data_points"])
...
def get_open_interest(symbol):
    ...
    try:
        ...
        return {"open_interest": float(open_interest) if open_interest else 0, "timestamp": oi_info.get("timestamp", 0)}
    except Exception as e:
        ...
        with activate_context(ctx):
            try:
                ...
                response = ai_client.chat.completions.create(
                    model=AI_MODEL, messages=[{"role": "user", "content": "Hello"}], max_tokens=10, timeout=10.0
                )
...
def analyze_with_deepseek(symbol, price_data, config):
    ...
    # 获取资金费率和持仓量信息，添加到price_data中供prompt_builder使用
    try:
